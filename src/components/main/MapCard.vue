<template>
	...
	<div class="map cursor-pointer" @click="this.$router.push(`/MapCanvas/${map._id}`)" @mouseenter="showOpt=true"
		...
		<div class="top-options" v-if="showOpt">
		    <div @mousedown="this.$emit('showEditMapWin', map)" title="Изменить">
			<svg class="cursor-pointer option svg-orange" stroke="#1C274C" viewBox="0 0 24 24" fill="none"
		    ...
		    <div @mousedown="this.$emit('showDelMapWin', map)" title="Удалить">
			<svg class="cursor-pointer option svg-orange" stroke="#1C274C" viewBox="0 0 24 24" fill="none"
			     xmlns="http://www.w3.org/2000/svg">
		...
		<img src="@/assets/images/plug.jpg" class="top-image" alt="...">
	    </div>
