<template>
	...
	<div v-if="showProfile" class="invisibleContainer"
		...
		<div class="flex-row">
			...
			<button class="button-light button-middle" type="button" @click="this.$store.dispatch('logOut')">Выход</button>
		    </div>
		    <div class="user-info">
...
.logo-text
  font-family: a_RewinderMedium,serif
  font-size: 28pt
  user-select: none
  margin-right: 25px
  margin-left: 10px
  color: Variables.$dark-color
  text-shadow: -2px -2px 0 Variables.$orange-color, 2px -2px 0 Variables.$orange-color, -2px 2px 0 Variables.$orange-color, 2px 2px 0 Variables.$orange-color
</style>
